...
{
    public class DelSentenceManageController : ApiController
    {
        ...
        public ResultInfoT<object> Post()
        {
            ...
            try
            {
                ...
                using (var db = new livecloudEntities())
                {
                    ...
                    var query = from w in db.dt_SensitiveSentences
                                where idList.Contains(w.id)
                                select w;

                    db.dt_SensitiveSentences.RemoveRange(query);

                    var manageLog = new dt_ManageLog()
                    {
                        ManagerId = managerId,
                        ManagerName = db.dt_Manager.Find(managerId).user_name,
                        ActionType = "delSentenceManage",
                        AddTime = now,
                        Remarks = "删除黑白名单ID："+ ids,
